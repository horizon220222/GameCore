...
void Game::run()
{
    while (is_running_)
    {
        ...
        if (elapsed < frame_delay_)
        {
            ...
            dt_ = static_cast<float>(static_cast<double>(frame_delay_) / 1.0e9);
        }
        else
        {
            dt_ = static_cast<float>(static_cast<double>(elapsed) / 1.0e9);
        }
    }
}
...
void Game::drawBoundary(const glm::vec2& top_left, const glm::vec2& botton_right, float boundary_width,
    SDL_FColor fcolor)
{
    SDL_SetRenderDrawColorFloat(renderer_, fcolor.r, fcolor.g, fcolor.b, fcolor.a);
